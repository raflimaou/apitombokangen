    ...
    if ($result_cek_cart) {
        ...
        if (mysqli_query($connection, $insert_invoice)) {
            ...
            while ($row_cart = mysqli_fetch_array($query_insert_cart_detail)) {
                ...
                $query_product = mysqli_query($connection, "SELECT * FROM product WHERE id_product = $idProduct");
                $jumlah = 0;
                while ($row_product = mysqli_fetch_array($query_product)) {
                    $jumlah = $row_product['jumlah'] + 1;
                    mysqli_query($connection, "UPDATE product SET jumlah = $jumlah WHERE id_product = $idProduct");
                }
            }
